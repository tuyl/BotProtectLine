...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + "rename dedek kak")
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " minggato\nyang jauh ya")
    except Exception as e:
        print e
        print ("\n\nNOTIFIED_KICKOUT_FROM_GROUP\n\n")
        return
            ...
            if msg.contentType == 0:
                ...
                if msg.text == "Check":
                    if msg.to in wait['readPoint']:
                        ...
                        sendMessage(msg.to, "Read Point Belum Di Atur")
#-------------------------------------------------------------
		if msg.text == "Speed":
                    start = time.time()
